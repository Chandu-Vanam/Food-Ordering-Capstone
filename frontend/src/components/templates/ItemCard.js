...
const ItemCard = ({ item, vendor, computeRating, onEdit, onDelete }) => {
    ...
    const imageUrl = `http://localhost:8000/public/images/${item.image}`;

    useEffect(() => {
        if (user_type() === 'buyer') {
            axios
                .get(`http://localhost:8000/api/buyers/details`, {
                    headers: {
    ...
    const handleFavouriteAdd = () => {
        axios
            .patch(`http://localhost:8000/api/buyers/add_favourite`, {
                item_id: item._id
            }, {
                headers: {
                    authorization: localStorage.getItem('token')
